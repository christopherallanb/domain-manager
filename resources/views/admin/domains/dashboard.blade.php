...
@section('content')
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        @php
            ...
            $in30 = $domains
                ->filter(
                    fn($d) => \Carbon\Carbon::parse($d->expiration_date)->diffInDays(now()) <= 30 &&
                        \Carbon\Carbon::parse($d->expiration_date)->isFuture(),
                )
                ->count();
            $in7 = $domains
                ->filter(
                    fn($d) => \Carbon\Carbon::parse($d->expiration_date)->diffInDays(now()) <= 7 &&
                        \Carbon\Carbon::parse($d->expiration_date)->isFuture(),
                )
                ->count();
        ...
        <div class="rounded-xl border bg-card p-5 flex items-center gap-4">
            <div class="rounded-xl p-3 bg-primary/10"><svg class="h-6 w-6 text-primary" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
            ...
            <div>
                ...
                <p class="text-xs text-muted-foreground">Total de domínios</p>
            </div>
        </div>
        <div class="rounded-xl border bg-card p-5 flex items-center gap-4">
            <div class="rounded-xl p-3 bg-warning/10"><svg class="h-6 w-6 text-warning" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div>
                <p class="text-2xl font-bold">{{ $in30 }}</p>
                <p class="text-xs text-muted-foreground">Vencendo em 30 dias</p>
            </div>
        </div>
        <div class="rounded-xl border bg-card p-5 flex items-center gap-4">
            <div class="rounded-xl p-3 bg-orange/10"><svg class="h-6 w-6 text-orange" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </svg></div>
            <div>
                <p class="text-2xl font-bold">{{ $in7 }}</p>
                <p class="text-xs text-muted-foreground">Vencendo em 7 dias</p>
            </div>
        </div>
        <div class="rounded-xl border bg-card p-5 flex items-center gap-4">
            <div class="rounded-xl p-3 bg-danger/10"><svg class="h-6 w-6 text-danger" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @foreach ($domains->sortBy('expiration_date')->take(5) as $domain)
                    ...
                    <div class="flex items-center justify-between gap-2 p-2 rounded-lg hover:bg-muted/50">
                        <div>
                            ...
                            <p class="text-xs text-muted-foreground">{{ $domain->registrar }}</p>
                        </div>
        ...
        <div class="rounded-xl border bg-card p-5">
            ...
            <div class="space-y-2">
                ...
                @foreach ($costByRegistrar as $registrar => $cost)
                    <div class="flex justify-between text-sm"><span>{{ $registrar }}</span><span class="font-medium">R$
                            {{ number_format($cost, 2, ',', '.') }}</span></div>
                @endforeach
            </div>
            <div class="mt-4">
