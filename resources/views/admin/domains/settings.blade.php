...
<div class="rounded-xl border bg-card p-5">
    ...
    <div class="space-y-6">
        <div class="rounded-lg border p-4">
            ...
            <form action="{{ route('domains.settings.save') }}" method="POST">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <input type="text" name="alert_emails" value="{{ old('alert_emails', $settings['alert_emails'] ?? '') }}" class="w-full rounded-md border border-input bg-background px-3 py-2 text-sm" placeholder="admin@domain.com, financeiro@domain.com">
                    ...
                    <div>
                        ...
                        <input type="text" name="alert_days" value="{{ old('alert_days', $settings['alert_days'] ?? '30,15,7,1') }}" class="w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
                        <p class="text-xs text-muted-foreground mt-1">Separe os valores por vírgula</p>
                    </div>
                    
                    <div class="flex items-center gap-2">
                        <input type="checkbox" name="email_notif" id="email_notif" {{ (isset($settings['email_notif']) && $settings['email_notif']) ? 'checked' : '' }}>
                        <label for="email_notif">Notificações por e-mail</label>
                    </div>
                    
                    <div class="flex items-center gap-2">
                        <input type="checkbox" name="telegram_notif" id="telegram_notif" {{ (isset($settings['telegram_notif']) && $settings['telegram_notif']) ? 'checked' : '' }}>
            ...
            @if(session('success'))
                <script>window.addEventListener('DOMContentLoaded', function(){ alert('{{ session('success') }}'); });</script>
            @endif
        </div>
